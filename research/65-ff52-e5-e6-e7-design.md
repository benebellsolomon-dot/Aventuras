...
## Deferred UI pass (2026-08-22 evening, worktree hardening-done-cr1-next-3f63ed)
The three "accepted / deferred" UI pieces above shipped together; nothing engine- or persistence-side changed (no template or sync bump):
- **E5 read-only notebook view** — `components/world/GmNotebookSection.svelte`, mounted at the bottom of the Quests tab (`QuestPanel.svelte`), gated on `settings.gmNotebook === true`. Collapsed header "GM's notebook (n)"; rows = kind icon (pin = reminder, waypoints = thread) + text + relative age (`this turn` / `n turns ago`); empty state when the toggle is on but nothing is stored; explicit "read-only — steer by playing" note. Reads through `readGmNotebook(self.metadata)` (already sanitizing), no writes. Home chosen over a new sidebar tab: the tab strip is fixed and already tight at 8 icons in BE mode; the notebook is continuity/threads, next to quests.
- **E6 VN-view inner voices** — `VnView.svelte` derives `resolveInnerVoices(latestNarration.content, story.characters)` once the beat is finished (`!ui.isStreaming`); an "Inner voices (n)" button sits bottom-left of the textbox (stopPropagation — the textbox click advances the beat) and toggles the list inside the textbox's scroll area; `voicesOpen` resets per beat in the existing `beatKey` effect. The feed/VN filter now lives in ONE helper `resolveInnerVoices` (`utils/thoughtTagParser.ts`, tested) — StoryEntry uses it too. The `npcThoughts` toggle therefore stays visible in VN mode.
- **E7 title-award notice** — `story.svelte.ts` sheet-apply path toasts `Title earned: <names> — see the Sheet tab` (info, 6 s) right where `titles awarded` is logged; the Sheet panel remains the durable record (no turn-log row — that would need a persisted record; skipped as YAGNI).
- Story Settings copy now says where each surfaces. Suite 1475, check/lint clean.

## Live findings (2026-08-23 00:00–00:30Z, read-only DB pull during Ben's session)

- **E6 `<thought>` — zero sightings in 60+ turns, ROOT CAUSE found and backstopped.** The live story ("Amelia of Evermere", `npcThoughts: true`) runs on `pack_id e4fd7d3b` = Ben's customized "Breast Expansion (NSFW)" pack. Its `adventure` template is a user fork that predates E6 (placeholder diff vs the code-synced Default template: exactly one item — `{{ npcThoughtInstructions }}` missing; it adds its own `beFocus`), and CR-2's version-gated sync correctly leaves user-modified templates alone — so the instruction was built every turn and silently dropped. Thoughts are stripped only at render (`stripThoughtTags` in StoryEntry/VnView/Streaming), so stored content is a valid probe: 0 narrations across ALL stories contain `<thought`. **Shipped:** `ensureNpcThoughtInstructions(systemPrompt, instructions)` in NarrativeService — pure; unchanged when the setting is off (empty instructions) or when the template already placed them; otherwise appended after the rendered system prompt (both the pack-template and per-story custom-prompt paths). Settings-off prompts byte-identical; templates that have the placeholder byte-identical. Three unit tests. Ben can alternatively add `{% if npcThoughtInstructions and npcThoughtInstructions != '' %}{{ npcThoughtInstructions }}{% endif %}` to the pack's `adventure`/`creative-writing` templates to place it where he wants it. Same trap for any FUTURE narrative-template placeholder on a customized pack — worth a general answer later (a sync that appends missing new placeholders, or a Settings warning when the active pack's template lacks a setting's placeholder). Verify live: next turn after the dev app restart should be able to emit `<thought who="Amelia">` (the InnerVoices panel shows it).
- **E7 titles — working as designed, nothing earned yet.** `titlesEarned: []` is present on every classified turn (the schema extension IS active — `rpgTitles` + `beMode` on), and the schema says "almost every response earns nothing"; the live story is an extended bedroom scene with no named feat. No action.
