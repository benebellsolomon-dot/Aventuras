r"""IPA nude tone fix — apply IPAdapter-face only AFTER structure/lighting forms (start_at).

Lowering weight alone didn't remove the nude washout. The real lever for IPA over-influencing
global exposure is start_at: let the first ~30% of denoise set structure+lighting WITHOUT IPA,
then apply face identity. Test curvy nude, two configs, vs the washed w0.8 baseline + non-IPA suite.
"""
...
MANIFEST = r"D:\LLM\ComfyUI_windows_portable\quality-upgrade\ipa_tonefix_manifest.json"
...
BASE_TIER = 14
TIERS = [0, 20, 33, 50]
# (tag, weight, start_at, embeds_scaling)
CONFIGS = [("s30", 0.7, 0.30, "V only"), ("s20kv", 0.7, 0.20, "K+V")]
...
def build(tag, weight, start_at, scaling):
    g = {}
    saves = {}
    ...
    for t in TIERS:
        ...
        pfx = "%s_t%d" % (tag, t)
        m = model(pfx, sl, hy)
        g[pfx + "_ipa"] = {"class_type": "IPAdapterAdvanced", "inputs": {"model": m, "ipadapter": ["ipam", 0], "image": [tag + "fc", 0], "clip_vision": ["clipv", 0], "weight": weight, "weight_type": "linear", "combine_embeds": "concat", "start_at": start_at, "end_at": 1.0, "embeds_scaling": scaling}}
        p = enc(pfx, cup)
        ...
        label = "IPATONE_%s_t%d" % (tag, t)
        g[pfx + "_save"] = {"class_type": "SaveImage", "inputs": {"images": [pfx + "_vd", 0], "filename_prefix": label}}
        saves[pfx + "_save"] = {"cfg": tag, "weight": weight, "start_at": start_at, "scaling": scaling, "tier": t, "label": label}
    return g, saves
...
chains = []
for tag, weight, start_at, scaling in CONFIGS:
    g, saves = build(tag, weight, start_at, scaling)
    r = post("/prompt", {"prompt": g})
    chains.append({"tag": tag, "prompt_id": r["prompt_id"], "saves": saves})
    print("queued", tag, "w", weight, "start_at", start_at, scaling, r["prompt_id"])
t0 = time.time()
...
manifest = {"tiers": TIERS, "configs": CONFIGS, "frames": []}
for ch in chains:
