"""v8 — final tuning of the HandRefiner pass: recover the contact deformation that v7's
re-render muted, while keeping the finger-geometry fix.
Changes vs v7: HandRefiner denoise 0.7->0.5; tighter mask (mask_expand 8->3, padding 30->16);
controlnet strength 0.8->0.85. Order unchanged: contact detailer -> HandRefiner (HR last).
3 contact cells; baselines = v7 (deformation recovery?) and v6 (fingers still fixed?)."""
import json, time, urllib.request, os
API="http://127.0.0.1:8188"; OUTDIR=r"D:\LLM\ComfyUI_windows_portable\ComfyUI\output"
MANIFEST=r"D:\LLM\ComfyUI_windows_portable\quality-upgrade\testsuite_v8_manifest.json"
...
def handrefiner(g, src):
    g["mg"]={"class_type":"MeshGraphormer+ImpactDetector-DepthMapPreprocessor","inputs":{"image":src,"bbox_detector":["hdet",0],"bbox_threshold":0.5,"bbox_dilation":10,"bbox_crop_factor":3.0,"drop_size":10,"mask_bbox_padding":16,"mask_type":"based_on_depth","mask_expand":3,"rand_seed":88,"resolution":512}}
    g["cnet"]={"class_type":"ControlNetLoader","inputs":{"control_net_name":"illustriousXLDepth_v20.safetensors"}}
    g["cnap"]={"class_type":"ControlNetApplyAdvanced","inputs":{"positive":["pos",0],"negative":["neg",0],"control_net":["cnet",0],"image":["mg",0],"strength":0.85,"start_percent":0.0,"end_percent":1.0}}
    ...
    g["ksh"]={"class_type":"KSampler","inputs":{"seed":S,"steps":20,"cfg":5.0,"sampler_name":"dpmpp_2m","scheduler":"karras","denoise":0.5,"model":["ckpt",0],"positive":["cnap",0],"negative":["cnap",1],"latent_image":["mask2",0]}}
...
def build(c):
    ...
    g["hfd"]=hand_detailer(["vdf",0]); img=handrefiner(g,["hfd",0])
...
manifest={"cells":[]}
print("submitting",len(CELLS),"v8 cells")
for c in CELLS:
    try:
...
for cell in manifest["cells"]:
    ...
    try:
        ...
        cell["files"]=[os.path.join(OUTDIR,im.get("subfolder",""),im["filename"]) for nid,o in outs.items() for im in o.get("images",[])]; cell["status"]=e.get("status",{}).get("status_str")
