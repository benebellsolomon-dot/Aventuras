"""Preset v3 — folds in all confirmed v2+v3 wins.
vs v2: default prompt gains (deep cleavage:1.2)+inter-breast shadow; negatives gain uniboob +
broadened prop/cleavage-object + specular-streak blocks; hand-detailer mask tightened (dilation 4, crop 2.0)."""
import json

POS = ("masterpiece, best quality, amazing quality, very aesthetic, newest, absurdres, highres, "
       "1girl, solo, mature female, adult, long hair, huge breasts, (deep cleavage:1.2), cleavage, "
       "curvy, wide hips, narrow waist, collarbone, fitted dress, "
       "indoors, standing, smile, soft lighting")  # P1 fix: dropped 'between breasts'/'inter-breast shadow' (object-in-cleavage prior)
...
def fd(wildcard):
    return [512, True, 1024, 0, "fixed", 20, 5.0, "dpmpp_2m", "karras", 0.45, 5, True, True, 0.5, 4, 2.0,
...
NODES = {
 ...
 6:  ("KSampler",[760,40],[("LATENT","LATENT")],[("model","MODEL"),("positive","CONDITIONING"),("negative","CONDITIONING"),("latent_image","LATENT")],[111,"fixed",28,5.0,"dpmpp_2m","karras",1.0],0),
 7:  ("VAEDecode",[1120,40],[("IMAGE","IMAGE")],[("samples","LATENT"),("vae","VAE")],[],0),
 8:  ("SaveImage",[1120,300],[],[("images","IMAGE")],["WAI_v17_v3_base"],0),
 ...
 13: ("KSampler",[1680,40],[("LATENT","LATENT")],[("model","MODEL"),("positive","CONDITIONING"),("negative","CONDITIONING"),("latent_image","LATENT")],[111,"fixed",20,5.0,"dpmpp_2m","karras",0.4],0),
 ...
 18: ("FaceDetailer",[2720,40],FD_OUT,FD_IN,fd(""),4),
 19: ("SaveImage",[3080,40],[],[("images","IMAGE")],["WAI_v17_v3_final"],0),
}
LINKS = [
...
for order,(nid,(ntype,pos,outs,ins,wv,mode)) in enumerate(NODES.items()):
    ...
    nodes_json.append({"id":nid,"type":ntype,"pos":pos,"size":[320,200],"flags":{},"order":order,"mode":mode,"inputs":inputs,"outputs":outputs,"properties":{"Node name for S&R":ntype},"widgets_values":wv})
...
print("WROTE v3 preset | nodes",len(nodes_json),"links",len(links_json),"| widget mismatches:",bad if bad else "none")
