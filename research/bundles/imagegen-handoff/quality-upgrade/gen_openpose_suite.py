r"""Full OpenPose tier suite — 4 body types x clothed/nude, the WORKING body-hold method.

Each combo is one graph: base txt2img -> DWPose skeleton -> per-tier txt2img with OpenPose CN
(full denoise -> unmuted growth; CN + body-pin tags hold pose/build; no chaining -> clean quality).
Replaces the failed chained-img2img suite (tier_suite_img2img).
"""
...
MANIFEST = r"D:\LLM\ComfyUI_windows_portable\quality-upgrade\openpose_suite_manifest.json"
    ...
    for t in TIERS:
        ...
        g[pfx + "_cna"] = {"class_type": "ControlNetApplyAdvanced",
                           "inputs": {"positive": p, "negative": ["neg", 0], "control_net": ["cn", 0],
                                      "image": ["pose", 0], "strength": CN_STRENGTH, "start_percent": 0.0, "end_percent": 1.0}}
        ...
        label = "OPS_%s_%s_t%d" % (body, clothing, t)
...
chains = []
print("submitting", len(BODY_ORDER) * len(CLOTHING_ORDER), "OpenPose graphs")
...
manifest = {"cn": CN_NAME, "tiers": TIERS, "body_order": BODY_ORDER, "clothing_order": CLOTHING_ORDER, "frames": []}
for ch in chains:
