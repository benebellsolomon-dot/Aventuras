r"""FULL body-hold suite — OpenPose + IPAdapter-FACE(face-crop), 4 builds x clothed/nude.

The complete recipe per combo:
  OpenPose CN (pose lock) + body-pin/anti-thick (build lock) + IPAdapter PLUS FACE referencing a
  FACE CROP of the base (identity lock) + full-denoise txt2img per tier (full growth).
Each combo = one graph: base txt2img -> base image -> {face crop, DWPose skeleton};
per tier: model patched with IPA-face(facecrop), conditioning patched with OpenPose pose.
"""
...
MANIFEST = r"D:\LLM\ComfyUI_windows_portable\quality-upgrade\openpose_ipa_suite_manifest.json"
...
def build_graph(body, clothing):
    ...
    for t in TIERS:
        ...
        label = "OPS2_%s_%s_t%d" % (body, clothing, t)
        g[pfx + "_save"] = {"class_type": "SaveImage", "inputs": {"images": [pfx + "_vd", 0], "filename_prefix": label}}
        saves[pfx + "_save"] = {"body": body, "clothing": clothing, "tier": t, "cup": cup,
...
chains = []
print("submitting", len(BODY_ORDER) * len(CLOTHING_ORDER), "OpenPose+IPA graphs")
...
manifest = {"cn": CN_NAME, "ipa_weight": IPA_WEIGHT, "tiers": TIERS,
