...
| [55-image-gen-identity-anchoring-design.md](55-image-gen-identity-anchoring-design.md) | **Image-gen identity anchoring (design + implementation record)**: per-character locked `imageTags` banks via unified `extractIdentity`, creation-time hygiene, review-gated backfill, aspect-ratio by shot type, negatives merge, emphasis gating, and the dedicated booru scene-prompt writer follow-up (narration LLM writes prose in `<pic>`; a focused LLM call rewrites it into Danbooru tags) | ✅ SHIPPED (Phases 1a/1b/2 + writer, commits fb5daba4→44534208) |
| [56-cr1-merge-review-findings.md](56-cr1-merge-review-findings.md) | **CR-1 merge review ledger**: 3-lens Opus adversarial review of merge `99cd8f79` (CR-1 atomic-turn-writes × image-gen/RPG line). 2 critical (batch stealing → torn turn reports success; withTransaction poison pill) + hygiene/`executeDirect` violation, saveBackground buffered, refreshWorldState mid-batch clobber, Linux pool-path mismatch — all FIXED same session; D-1..D-13 deferred backlog (stop-ordering, world-panel gating during turns, background-writer routing, beMode-without-tracking legacy path, sprite-effect reorder, durability posture, narration orphan UX) + cleared-as-correct list | ✅ FIXES LANDED; 📋 D-1..D-13 backlog |
| [58-ff52-port-brainstorm.md](58-ff52-port-brainstorm.md) | **FF5.2 → Aventuras port brainstorm** (full preset read + codebase map): three-layer taxonomy (prose modules / Internal States / CoT scaffolds), ST-emulation → engine translation table, P/E bucket mapping, Ben's D1–D4 rulings (cinematic default; UNIFY harem bond onto −5..+20 BOND; E1→E3+E4→E2; NSFW both) | Rulings recorded |
| [59-ff52-phase1-plan.md](59-ff52-phase1-plan.md) | **FF5.2 Phase 1 plan (prose + settings package)**: template rewrite (cinematic/literary proseStyle, hybrid POV, anti-echo, bold NPCs, knowledge/perception, VAD inner life, NPC genesis, merged ban lists), responseLength + nsfwFlavor settings, explicit-rating delivery layer; engine phases pointed to future docs | ✅ IMPLEMENTED (this branch, suite 1117) |
| [60-ff52-phase2-plan.md](60-ff52-phase2-plan.md) | **FF5.2 Phase 2 (unified relationship engine)**: harem bond 0–100 → −5..+20 `rel` block (bond/sparks/grudge/ct) with floor(÷5) lazy legacy conversion preserving every band+gate edge; earned-only bond via sparks conversion (grudge stall, remainder banking, warm-repair, potent spell events); gates 9/14; hostile/cold bands; 3-lens adversarial review outcome recorded in-doc | ✅ IMPLEMENTED + reviewed (this branch) |
| [61-ff52-phase3-plan.md](61-ff52-phase3-plan.md) | **FF5.2 Phase 3 (E3 world sim + E4 off-screen agendas)**: stateless seeded event tables (Standard/Duo, sparse/lively gate, arousal suppression), `npcAgenda` character metadata + classifier proposals + deterministic active-cast ticking + completion effects as capped bond events, [OFF-SCREEN]/[WORLD EVENT] prompt-tail blocks, byte-identical when off; 3-lens review + fix-diff round recorded in-doc | ✅ SHIPPED (4f9ea495, suite 1222) |
| [62-ff52-phase4-plan.md](62-ff52-phase4-plan.md) | **FF5.2 Phase 4 (E2 Chekhov's Gun narrative debt)**: story-scoped bullet state hosted on the self character's metadata (storage ruling in-doc), classifier-observed loading/resolution (`narrativeDebt`/`resolvedDebts`), deterministic seeded firing (FF thresholds 18/13/8, age mods, proximity, urgency, floor-2 jam), one-turn [CALLBACK] block, fired-unresolved re-load with refractory, CALM-plant + research-seed hooks, `chekhovGun` toggle byte-stable off; review outcomes recorded in-doc | ✅ IMPLEMENTED + reviewed (this branch) |
| [63-d5-playtest-round1.md](63-d5-playtest-round1.md) | **D5 calibration playtest round 1**: seven fixed findings (story-setting image context, silent-choices triple root cause incl. hard-schema gates + minimax markdown retry, MEASURED NanoGPT 77-token CLIP truncation → provider-aware tag budget, dagger→rifle generic-tag fix, SFW-scene rating grounding, POV camera guidance) + the open classifier entity-routing muddle handed to the next session | ✅ Fixes shipped; routing chip open |
| [57-growth-coherence-design.md](57-growth-coherence-design.md) | **Growth coherence (rounds 3-8 design record)**: one player-visible roll — guaranteed cast/check growth (no second reducer dice), degradation ladders for every LLM-tagged field (inferred target, effectivePresence, unknown-spell drop), pre-flight growth verdict in [CHECK RESULT], crit pierce + earned banking (Ben's ruling), whitelist-and-inject image size, growth_magnitude prose drift arm, honest refusal UI, actInProgress validation-retry. Plus the meta-lessons (deterministic composition > template pleading; blacklists lose to phrasing variance) | ✅ SHIPPED (commits 805044b4→dd1a2f34, suite 1099) |
