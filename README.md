...
- **Per-character image-tag bank** — curated, physical-only identity tags that lock a character's look across every generated image
...
- **Anti-bleed multi-character prompts** — structured guidance (count tag, per-character clauses with spatial anchors) to keep characters visually distinct
...
- Configurable image size and per-task profiles (main / portrait / reference / sprite)
...
- Size grounding for images, banded sprite sets, and tier-scaled LoRA weighting all read from the engine
