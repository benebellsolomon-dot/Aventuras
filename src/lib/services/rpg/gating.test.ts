...
describe('availableInteractions gates (both sides of each boundary)', () => {
  ...
  it('milking needs bond AND an active supply', () => {
    expect(availableInteractions(input({ bond: 45 })).available).not.toContain('milking')
    expect(
      availableInteractions(input({ bond: 44, lactationActive: true })).available,
    ).not.toContain('milking')
    expect(availableInteractions(input({ bond: 45, lactationActive: true })).available).toContain(
      'milking',
    )
  })

  it('induce_lactation needs bond and is spent once she is lactating', () => {
    expect(availableInteractions(input({ bond: 44 })).available).not.toContain('induce_lactation')
    expect(availableInteractions(input({ bond: 45 })).available).toContain('induce_lactation')
    expect(
      availableInteractions(input({ bond: 45, lactationActive: true })).available,
    ).not.toContain('induce_lactation')
  })

  it('inducing and milking are mutually exclusive offers', () => {
    const before = availableInteractions(input({ bond: 60 })).available
    const after = availableInteractions(input({ bond: 60, lactationActive: true })).available
...
describe('buildGatedActionsInstruction', () => {
  ...
  it('lists per-girl availability and the hard rule', () => {
    const instruction = buildGatedActionsInstruction([
      input({ name: 'Mira', bond: 75, dependence: 40 }),
      input({ name: 'Sable' }),
    ])
  ...
  it('a lactating girl advertises her supply band alongside her offers', () => {
    const instruction = buildGatedActionsInstruction([
      input({ name: 'Mira', bond: 75, lactationActive: true, supplyTier: 3 }),
    ])
