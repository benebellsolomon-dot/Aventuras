...
FIELD "characters": an ARRAY — one entry per person, in the SAME order as the count tags, each a FLAT comma-separated tag run: identity tags → clothing → breast-size band. NO expression tags here; they have their own field. Never wrap a run in parentheses — booru models have no regional prompter, so a parenthesized clause is only emphasis and pushes the rest of the prompt out of the attention window.
  - A person in the "Named subjects" list: copy their LOCKED IDENTITY TAGS VERBATIM (do not alter, reorder, or drop them). If they have no locked tags but an APPEARANCE line, convert that prose into atomic booru tags (e.g. "long hair, wavy hair, blonde hair, blue eyes, fair skin"). Then add their DRESS STATE for this beat and their body line from the dossier. The dress state is the dossier's current clothing while the beat keeps it on; when the scene intent or the beat says it is off or displaced, it is the Danbooru dress-state tags of THIS beat instead — "completely nude" / "topless" / "bottomless", plus the displacement tag for where the clothes went ("clothes pull", "shirt lift", "panties around one leg") — and never "clothed". Every described run states exactly one dress state; when the intent says she is bare and her run states none, the output is rejected automatically and you write it again.
...
- Copy the dossier's BODY line verbatim, and its CURRENT CLOTHING verbatim while she still wears it — never re-invent an outfit or a body size. But the clothing line describes what she WORE when the beat began, and the beat wins: "standing bare" / "naked" / "breasts exposed" in the scene intent means nude-family tags in her run, whatever the dossier says. The body line is already in tag form; copy it, do not paraphrase it into prose. Every female subject's run carries a breast-size band matching the dossier (flat chest / small breasts / medium breasts / large breasts / huge breasts / gigantic breasts / hyper breasts), and her clothing matches that size — at large sizes clothes strain, gape, or fail.
...
- ORDER: (1) count and subjects first — "one woman", "two women", "a man and a woman" — then each person IN FULL: age bracket, race/species if not human, skin tone, eye color, hair length/style/color, build, the breast-size band word given in the dossier (canon — always state it for a woman), her clothing AS IT IS in this beat (the dossier's current clothing while she wears it; when the scene intent says it is off, say plainly that she is naked or topless and where the garment went), a CONCRETE facial expression ("teeth bared in a fierce grin", "eyes wet, lips trembling" — never "neutral", never "emotional"), posture and held items. With several people, each gets their OWN sentence with a spatial anchor ("on the left, …; on the right, …; behind her, …") — never interleave two people's traits. (2) What they are doing and how they relate in space; an act already under way is described as under way. (3) Camera: shot type and angle in plain phrases ("medium shot from a slight low angle", "close-up", "shallow depth of field"). (4) Environment and light last: setting, time of day, the NAMED light source and its quality, atmosphere, and named objects with their placement.
- IDENTITY ANCHORS: when a subject's dossier lists locked identity tags, restate every one of them in plain words — hair length, style and color; eye color; skin tone; species or permanent marks — even when the scene intent omits them; that is what keeps the same person recognizable from picture to picture. Where the scene intent and the dossier disagree on hair, eyes or skin, the dossier wins.
