...
const snapAt = (tier, fill = 0) =>
  C.compute_body_snapshot({
    tier_index: tier,
    breast_shape: 'natural',
    height_cm: REF_H,
    build: 'average',
    fill_percent: fill,
    lactation_active: fill > 0,
    fluid_type: 'milk',
  })
...
console.log(
  'tier | letter | dry/side kg | vol/side ml | r_hemi cm | spine bust | hull bust | Δ(hull−spine)',
)
...
console.log(
  `frame estimate (H=${REF_H}, average): ${C.estimated_body_weight_kg(REF_H, 'average')} kg`,
)
const t47kg = 2 * dryKgSide(47)
console.log(
  `tier 47 dry tissue total: ${t47kg.toFixed(1)} kg → true total ≈ ${(Number(C.estimated_body_weight_kg(REF_H, 'average')) + t47kg).toFixed(1)} kg (frame + tissue, before fluid)`,
)
...
for (const [label, ml] of [
  ...
  console.log(
    `  ${label}: hull bust ${bust.toFixed(1)} cm (diff ${(bust - underbustCm).toFixed(1)} cm over underbust)`,
  )
}
